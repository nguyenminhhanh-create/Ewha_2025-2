...
DATA_FILE = "daily_trending_videos.csv"
...
def load_data():
    ...
    Filters for Korea region if possible.
    """
    if os.path.exists(DATA_FILE):
        try:
            df = pd.read_csv(DATA_FILE)
            # Basic preprocessing - adjust column names if necessary based on common Kaggle datasets
            # Expecting: trending_date, view_count, likes, comment_count, country
            
            # Normalize column names to lowercase
            df.columns = [c.lower() for c in df.columns]
            
            # Filter for Korea if 'country' or 'region' column exists
            ...
            # Ensure proper dates
            # trending_date format varies, often '2023-01-01' or '23.01.01' or '2023-01-01T00:00:00Z'
            # We try to convert flexibly
            df['trending_date'] = pd.to_datetime(df['trending_date'], format='mixed', utc=True).dt.date
            
            # Group by date to get daily totals (since regression needs time series)
